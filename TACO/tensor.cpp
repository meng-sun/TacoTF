...
//CHANGED THIS!!!
void TensorBase::setFormat(const Format& format){
...
//ADDED CODE HERE
std::vector<taco::ir::Stmt> TensorBase::get_lowers(bool assembleWhileCompute) {
  taco_uassert(getExpr().defined()) << error::compile_without_expr;
  ...
  //content->assembleFunc = lower::lower(*this, "assemble", assembleProperties);
  //content->computeFunc  = lower::lower(*this, "compute", computeProperties);
  std::vector<taco::ir::Stmt> statements (2);
  statements[0] = lower::lower(*this, "assemble", assembleProperties);
  statements[1] = lower::lower(*this, "compute", computeProperties);
  content->assembleFunc = statements[0];
  content->computeFunc = statements[1];
  return statements;
}

std::string TensorBase::functionize(std::vector<taco::ir::Stmt> statements) {
  //util::Timer conservedTimer;
  //conservedTimer.start();
  content->module->addFunction(statements[0]);
  content->module->addFunction(statements[1]);
  std::string path = content->module->compile();
  //conservedTimer.stop();
  //std::cout<<"\tconservedTimer:"<<conservedTimer.getResult()<<std::endl;
  //std::cout<<"path "<<path<<std::endl;
  return path;
}

void TensorBase::apply_funcs(string path, taco::ir::Stmt * assembleFunc, taco::ir::Stmt * computeFunc) {
  //util::Timer moduleCompileTimer;
  //moduleCompileTimer.start();
  //std::cout<<"apply path "<<path<<std::endl;
  //taco_uassert(getExpr().defined()) << error::compile_without_expr;
  ...
  content->assembleFunc = *assembleFunc;
  content->computeFunc  = *assembleFunc;
  content->module->addFunction(*assembleFunc);
  content->module->addFunction(*computeFunc);
  content->module->openPath(path);
  //moduleCompileTimer.stop();
  //std::cout<<"\tmoduleCompileTimer:"<<moduleCompileTimer.getResult()<<std::endl;
}
...
void TensorBase::assemble() {
  ...
  //assembleTimer.stop();
  //std::cout<<"\t\tloadAssembleCodeTimer:"<<assembleTimer.getResult()<<std::endl;
...
void TensorBase::compute() {
  ...
  //compileTimer.stop();
  //std::cout<<"\t\tloadCompileCode:"<<compileTimer.getResult()<<std::endl;
  if (content->assembleWhileCompute) {
    taco_tensor_t* tensorData = ((taco_tensor_t*)content->arguments[0]);
    content->valuesSize = unpackTensorData(*tensorData, *this);
  }
